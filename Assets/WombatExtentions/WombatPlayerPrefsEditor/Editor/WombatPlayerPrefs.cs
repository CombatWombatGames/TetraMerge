...
{
    public class WombatPlayerPrefs : EditorWindow
    {
        [MenuItem("Window/PlayerPrefs Editor")]
        static void Init()
        {
            EditorWindow.GetWindow(typeof(WombatPlayerPrefs)).titleContent = new GUIContent("PlayerPrefs");
        }
        ...
        //Ascending is A-Z or 1-2. Descending is Z-A or 2-1.
            ...
            if (!string.IsNullOrEmpty(SearchString))
            {
                for (int i = 0; i < ppeList.Count; i++)
                {
                    ...
                    if ((SearchFilter == SearchFilterType.Value || SearchFilter == SearchFilterType.All) && ppeList[i].Type == ValueType.String)
                    {
                        ...
                        {  //Prevent duplicates when 'All' is selected
